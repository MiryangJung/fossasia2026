# FOSSASIA Gallery - Project Guide

## Project Overview

FOSSASIA 발표를 위한 프로젝트. React Native(Expo)에서 대량 이미지 갤러리의 성능 최적화 사례를 다룸.

**Duration**: 15분

### Deliverables
1. **Slidev 슬라이드** (`/slides`) - 발표용 슬라이드 (sli.dev)
2. **Expo 데모 앱** (`/demo`) - 블로그 내용을 직접 구현하여 슬라이드 코드 근거로 활용
3. **발표자 스크립트** - 한국어/영어 병기, Slidev presenter notes에 포함

### Presentation Style
- 라이브 데모 없음 - 슬라이드에 코드 스니펫과 결과 데이터를 포함
- 데모 앱을 직접 구현하고 직접 측정한 수치를 슬라이드에 사용
- 블로그는 구현 방법론의 참고/영감 - 수치나 내용을 그대로 사용하지 않음

### Workflow
1. Expo 데모 앱 구현 (블로그 4단계 참고하여 직접 구현)
2. 실제 기기에서 성능 측정 (메모리, 로딩 시간 등)
3. 측정 데이터 + 코드 기반으로 Slidev 슬라이드 작성

## Project Structure

```
/slides          # Slidev presentation
  /slides.md     # Main slide deck with presenter notes (KR/EN)
  /script-en.md  # English speaker script
  /script-en-phonetic.md  # English script with Korean phonetic guide
  /components    # Custom Vue components for slides
  /public        # Images, GIFs, diagrams for slides

/demo            # Expo gallery demo app (Expo SDK 55, dev client)
  /app           # Expo Router pages (index, rn-image, expo-image, mipmaps)
  /components    # GalleryGrid, MeasureOverlay
  /hooks         # useMediaLibraryPhotos, useLoadTimeTracker, useMipmaps
  /scripts       # populate-simulator.ts (테스트 이미지 주입)
  /assets/photos # 시뮬레이터 주입용 원본 이미지 6장
```

## Presentation Topic

**Title**: "From 2GB to 200MB: Optimizing Large Image Lists in React Native"

**Abstract**: In-app photo picker에서 대량 이미지 렌더링 시 메모리 사용량이 2GB까지 치솟아 앱이 크래시되는 문제를 진단하고, 200MB 수준까지 최적화한 사례 발표.

**Reference**: https://blog.swmansion.com/react-native-image-list-recreating-apple-google-photos-in-react-native-part-1-7f73fb74fc63

### Implementation Steps (블로그 방법론 참고, 직접 구현)
1. **Step 1 - Loading**: expo-media-library 배칭 로드 + MMKV 캐싱
2. **Step 2 - Image Component**: `<Image>` vs `expo-image` 비교 구현
3. **Step 3 - Mipmaps**: ImageManipulator로 다운스케일 썸네일 생성
4. **Step 4 - List Component**: FlashList 사용 (슬라이드에서 FlatList/FlashList/LegendList 비교 설명)
5. **Techniques**: placeholder, blurhash, skeleton, cachePolicy 등

### Benchmarking
- 모든 수치는 직접 구현 후 실제 기기에서 측정
- 측정 항목: 메모리 사용량, 이미지 로딩 시간, 스크롤 FPS
- 데모 앱에 측정 유틸리티 포함

## Tech Stack

### Slides
- **Slidev** (sli.dev) - Markdown-based presentation tool
- Node.js / pnpm

### Demo App
- **Expo** (SDK 55)
- **expo-image** - 고성능 이미지 컴포넌트
- **expo-media-library** - 기기 이미지 접근
- **expo-image-manipulator** - 썸네일/mipmap 생성
- **@shopify/flash-list** - 고성능 리스트 (뷰 재활용)
- **react-native-mmkv** (v4, `createMMKV()` API) - 고성능 캐싱
- **expo-file-system** (v55, `Directory`/`File`/`Paths` API) - mipmap 파일 저장
- **react-native-nitro-modules** - mmkv 의존성
- **expo-router** - file-based routing
- TypeScript strict mode

## Conventions

### Slides
- Slidev presenter notes에 발표자 스크립트 작성 (한국어/영어 병기)
- 별도 스크립트 파일: script-en.md (영문), script-en-phonetic.md (영문+발음)
- 슬라이드는 간결하게, 키워드와 비주얼 중심
- 코드 예제는 핵심 부분만 하이라이트

### Demo App
- Expo Router (file-based routing) 사용
- 4개 모드: RN Image + FlashList / useImage + FlashList / expo-image + FlashList / Mipmaps + FlashList
- 성능 메트릭 오버레이 (MeasureOverlay) - 로드 시간, 이미지 수 표시
- 4열 그리드 (FlashList), 다크 테마
- dev client 빌드 (`npx expo run:ios`)
- 시뮬레이터 테스트 이미지: `npm run populate -- [count]`
- TypeScript strict mode

### General
- 커밋 메시지: 영어, conventional commits (feat:, fix:, docs:)
- 패키지 매니저: pnpm (slides), npm (demo)
- GitHub: https://github.com/MiryangJung/fossasia2026

## Slide Flow (15min, 18 slides)

1. Title (15s)
2. About Me (30s)
3. Motivation - 왜 in-app photo picker가 필요한가 (30s)
4. The Problem - RN Image + FlashList, RAM 2.3GB 피크 (50s)
5. How Other Gallery Apps Solve This - 인덱싱, 저해상도 먼저 (30s)
6. Batch Loading & Caching - expo-media-library + pagination (50s)
7. Rendering: RN Image - ph:// 문제, ~3,796ms (50s)
8. Rendering: useImage Trap - 메모리 누수, ~4,848MB (50s)
9. Rendering: expo-image - 네이티브 ph:// 처리, ~85ms (40s)
10. Decoding: Mipmaps 개념 - 8~24MP → 294px, ~190x (50s)
11. Decoding: Mipmap Size Calculation - PixelRatio, 캐시 키 (40s)
12. Decoding: Mipmap Generation & Caching - 파이프라인, MMKV (50s)
13. Decoding: Mipmaps Result - 174MB, 57ms (30s)
14. Scrolling: FlashList - 뷰 리사이클링 (40s)
15. Results - 4가지 접근법 비교 (30s)
16. Results Summary - 테이블 (30s)
17. Lessons Learned (50s)
18. Thank You (20s)

## SWMansion Blog Reference (영감/방법론 참고용, 수치 미사용)
- URL: https://blog.swmansion.com/react-native-image-list-recreating-apple-google-photos-in-react-native-part-1-7f73fb74fc63
- GitHub Repo: referenced in blog (SWM Photo app)
- Related Expo PRs: #37795, #37979, #37987 (expo-image iOS improvements)
- 블로그의 벤치마크 데이터는 memory/benchmark-data.md에 참고용으로 보관
